...
// this is all from Microsoft.Samples.SimplePDBReader.SymbolProvider
//let printPdbChecksums() =
...
//    let a = @"C:\Projects\scoutplugin\packages\SourceLink\lib\net45\SourceLink.pdb"
//    use s = File.OpenRead a
////    let r = TempPdbReader.CreateRawReader s
//    let r = TempPdbReader.Create s
//    for d in r.SymbolReader.GetDocuments() do
//        printfn "%s" d.URL
////        printfn "  %A" (d.GetCheckSum())

    // try using raw reader, but the COM imports files from Roslyn aren't good
    // They should match 
//    let mutable cDocs = 0
//    r.GetDocuments(0, &cDocs, null)
//    printfn "docs count: %d" cDocs
//    let mutable docs = Array.zeroCreate<Roslyn.Utilities.Pdb.ISymUnmanagedDocument> cDocs
//    r.GetDocuments(cDocs, &cDocs, docs)

//    for d in docs do
//        let sbUrl = StringBuilder 2048
//        let mutable cCh = 0
//        let r = d.__GetURL(0, &cCh, IntPtr.Zero)
//        let r = d.__GetURL(2048, &cCh, sbUrl)
//        printfn "r %d, cCh %d" r cCh
